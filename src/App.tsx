...
import { ActionStatus, defaultAction, defaultAPIAction } from './store/common/actions';
import { AppProvider } from './providers/AppProvider';
import React, { useState } from 'react';
import { PrivateRoute } from './components/PrivateRoute';
import { CookiesProvider } from 'react-cookie';
import { useSelectAuthData } from './store/staticReducers/authReducer/selectors';
import { getToken, setToken } from './store/staticReducers/authReducer/reducer';
import { configAxios } from './config/request';
import { LOG_IN, SET_NOTIFICATION_DATA } from './store/staticReducers/authReducer/types';
...
function App() {
    ...
    React.useEffect(() => {
        if (user.status == ActionStatus.Success && isLoggedIn && user.data != null) {
            defaultAPIAction({
                path: `/${ModelNamesEnum.Alert}/list/${user.data?.user_id}`,
                method: HttpMethod.GET,
                body: { token: getToken() },
                onFinish: (success, res) => {
                    if (success && res.data.response) {
                        defaultAction({
                            data: res.data.response.sort(
                                (al1: IAlertData, al2: IAlertData) => al2.alert_id - al1.alert_id
                            ),
                        })(dispatch, SET_NOTIFICATION_DATA);
                    }
                },
            })(dispatch);
        }
                        ...
                        defaultAPIAction({
                            ...
                            onFinish: (success, res) => {
                                if (success && res?.data?.response?.length > 0) {
                                    defaultAction({
                                        data: res.data.response[0].project_id,
                    ...
                    <BrowserRouter>
                        <NavBars renderSideBar={isLoggedIn} renderNavBar={isLoggedIn}>
                            <Switch>
                                ...
                                />
                                <PrivateRoute
                                    key={'participationManagement'}
                                    path="/participationManagement"
                                    component={ParticipantManagerPage}
                                    redirectTo={'/login'}
                                    isLoggedIn={isLoggedIn}
                                />
                                <PrivateRoute
