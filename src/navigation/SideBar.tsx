...
import { useSelectSideBarConfig } from '../store/staticReducers/commonReducer/selectors';
...
function MenuItem(props: IMenuItemProps) {
    ...
    return (
        ...
        >
            {renderIcon && <ListItemIcon>{listItem.icon}</ListItemIcon>}
            <Box
                style={{
                    display: 'flex',
                    flexGrow: 1,
    ...
    return (
        ...
        >
            ...
            >
                ...
                />
                {renderIcon && <ListItemIcon>{subItem.icon}</ListItemIcon>}
            </ListItemButton>
        </Box>
    );
}
                ...
                return (
                    <Box key={sectionIndex}>
                        {sections.map((listItem, listItemIndex) => {
                            if (
                                !listItem.requireAdmin ||
                                (listItem.requireAdmin &&
                                    user.data?.is_super_user)
                            ) {
                                return (
                                    <Box
                                        key={sectionIndex + '_' + listItemIndex}
                                    >
                                        <MenuItem
                                            listItem={listItem}
                                            isOpen={props.isOpen}
