...
import { Card, CardActions  , CardContent, CardHeader, Chip, FormControl, InputLabel, MenuItem, OutlinedInput, Select } from '@mui/material';
...
const Project = React.memo((props: IProjectProps) => {
    ...
    React.useEffect(() => {
        if(userList.status === ActionStatus.Success && projectUserLinks.status === ActionStatus.Success){
            const temp: number[] = [];
    ...
    return (
        <Card>
            <CardHeader
                title={<Title title={data.name} variant={'h5'} style={{display: 'inline-flex'}}/>}
                ...
                <GridContainer>
                    ...
                    <InputLabel shrink>Managers</InputLabel>
                        <Select
                            ...
                            value={selected}
                            onChange={event => {
                                const newValues = event.target.value as any as number[];
                            ...
                            renderValue={(selected) => (
                                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                                    {selected.map((userIndex) => {
                                        ...
                                        return (
                                            <Chip key={userIndex} label={`${user?.first_name} ${user?.last_name}`} />
                                        )}
                                )}
                                </Box>
                            )}
...
function ProjectDatatable(props: IProjectDatatableProps) {
    ...
    <Box>
        {props.data.map((project, projectIndex)=>(
            <Paper key={project.project_id} variant={'elevation'} elevation={3} style={{flexGrow: 1, marginBottom: '1em'}}>
