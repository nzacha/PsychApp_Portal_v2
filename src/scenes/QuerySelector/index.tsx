...
function downloadQuery(query: IQuizAnswerData[]) {
    ...
    query = query.sort(
        (el1, el2) => el1.index - el2.index || el1.question_id - el2.question_id
    );
    let data = [];
    ...
    donwloadLink.setAttribute(
        'download',
        'user_' +
            query[0].project_participant.authentication_code +
            '_answers.csv'
    );
...
const QuerySelector = React.memo(() => {
    ...
    const [participantCode, setParticipantCode] = useState<IFormItem<string>>(
        newFormItem('', false)
    );
    React.useEffect(() => {
        ...
        <Fade in={true} timeout={drawerTransitionTime}>
            ...
            >
                ...
                <Typography variant="h6" paragraph>
                    Here, you can view quiz answers
                </Typography>{' '}
                ...
                >
                    <TextField
                        ...
                        onBlur={(event) => {
                            setParticipantCode(
                                newFormItem(participantCode.value, true)
                            );
                        }}
                    />
                </Box>
                <Box
                    display={
                        answerList?.data?.length && answerList.data.length > 0
                            ? 'flex'
                            : 'none'
                    }
